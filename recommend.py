...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    nearest_centroid = lambda x: find_closest(restaurant_location(x), centroids)
    return group_by_first([[nearest_centroid(restaurant), restaurant]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    sxx = sum([(number - mean(xs)) ** 2 for number in xs])
    syy = sum([(rating - mean(ys)) ** 2 for rating in ys])
    x_addend = [(number - mean(xs)) for number in xs]
    y_addend = [(rating - mean(ys)) for rating in ys]
    sxy = sum([a * b for a, b in zip(x_addend, y_addend)])
    b = sxy / sxx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    features = [find_predictor(user, reviewed, feature)
                for feature in feature_fns]
    highest_r_squared = max(features, key=lambda x: x[1])
    return highest_r_squared[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            restaurant_dict[name] = user_rating(user, name)
        else:
            restaurant_dict[name] = predictor(restaurant)
